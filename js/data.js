...
/* ---------- MEL ----------
   ...
   Behandle dem deretter:
     ...
     kr          OBSERVERT DAGLIGVAREPRIS PER KILO, norsk marked, hentet
                 ...
                 TRE RADER STÅR IGJEN SOM ANSLAG, og de er det av natur — ikke
                 fordi ingen har lett:
                   «Sterkt hvetemel / manitoba (W300+)» — det finnes ingen norsk
                     forbrukervare i denne klassen. Møllerens og Regal lager den
                     ikke. Den realistiske veien er Caputo Manitoba (som ligger
                     som egen rad, verifisert) eller en 25 kg-sekk fra en
                     spesialforhandler. Tallet er satt nær Caputo.
                   «Knekt hvete» — selges ikke for seg i vanlig dagligvare.
                     Priset som Møllerens hvete helkorn, som er samme korn.
                   «Annet mel» — en plassholder for noe som ikke står i lista.
                     Den KAN ikke ha en observert pris; det er hele poenget
                     med den.
...
const PRIS_KILDE = 'Oda.no (kilopris per vare), Meny for 2 kg-posene av siktet hvetemel, og helsekostleddet for de kornslagene dagligvare ikke fører';
...
const FLOURS = [
  ...
  { id:'samalt_rug',      navn:'Sammalt rug fin',                   gruppe:'Rug',          protein:9.2,  absorpsjon:1.15, styrke:'ingen',         maxPct:25,  kr:16.4, grov:1,
  ...
  // Urkorn
  { id:'enkorn',          navn:'Enkorn (einkorn)',                  gruppe:'Urkorn',       protein:13.0, absorpsjon:0.90, styrke:'svært svak',    maxPct:35,  kr:85, grov:1,
  ...
  { id:'kikertmel',       navn:'Kikertmel',                         gruppe:'Andre korn',   protein:19.0, absorpsjon:1.04, styrke:'ingen',         maxPct:20,  kr:109, grov:1,
...
const SOAKERS = [
  ...
  { id:'havregryn', navn:'Havregryn',       kaldt:90,  varmt:206, type:'begge', behandling:'skald', korn:true,  kr:24.45, notat:'Størst forskjell kaldt/varmt av alt (+116 g). Slipper fuktighet som damp under steking.' },
  { id:'ruggryn',   navn:'Ruggryn / rugknekk', kaldt:65, varmt:154, type:'varmt', behandling:'skald', korn:true, kr:78, notat:'MÅ skåldes. Kaldbløtlagt rugknekk blir grus i brødet.' },
  { id:'knekt_hvete',navn:'Knekt hvete',    kaldt:178, varmt:225, type:'varmt', behandling:'skald', korn:true,  kr:23.1, prisAnslag:true, notat:'' },
  { id:'hvetekli',  navn:'Hvetekli',        kaldt:96,  varmt:168, type:'begge', behandling:'bloet', korn:true,  kr:36.75, notat:'Gråsone: oppfører seg nesten som mel. Vurder å telle den i melmengden.' },
  { id:'byggflak',  navn:'Byggflak',        kaldt:125, varmt:200, type:'varmt', behandling:'skald', korn:true,  kr:75, notat:'' },
...
const BAKE_PROFILES = [
  { id:'brod_gryte', lokket:true, navn:'Støpejernsgryte med lokk', anbefaltTil:'frittstående brød', vekt:'800–900 g', hydrering:'70–75 %',
  ...
  { id:'brod_apen', lokket:false, navn:'Åpen steking på stein/stål', anbefaltTil:'frittstående brød med eget dampkar', vekt:'800–900 g', hydrering:'70–75 %',
  ...
  { id:'brod_600', lokket:false, navn:'Åpen steking, mindre emner', anbefaltTil:'brød rundt 600 g', vekt:'600 g', hydrering:'70–75 %',
  ...
  { id:'brod_brett', lokket:false, navn:'Vanlig steking i ovnen', anbefaltTil:'stekebrett eller brødform, uten stein, stål eller gryte',
  ...
  { id:'brod_glass_stal', lokket:true, navn:'Glassgryte PÅ 15 mm stål', anbefaltTil:'frittstående brød med Pyrex + stål', vekt:'700–800 g', hydrering:'72–78 %',
  ...
  // termisk sprang, og en romtemperert klokke inn i 260-graders ovn er 235+.
  ...
  // trangt for et 900 g emne etter ovnsløftet — klokka er samme gryte.
  ...
  { id:'brod_kloke', lokket:true, navn:'På stålet, glasset som klokke over', anbefaltTil:'frittstående brød med Pyrex + stål', vekt:'700–800 g', hydrering:'72–78 %',
    ...
    notat:'Teoretisk det sterkeste oppsettet: full stålkontakt mot deigen OG et lukket dampkammer. Krever at gryta er dyp nok til å dekke brødet uten å røre det. King Arthurs egen sammenligning kåret nettopp «støpejern + kokende vann + opp-ned bolle over brødet» til vinner. MAKS 230 °C med Pyrex som klokke — glasset tåler 220 °C termisk sprang, så det skal aldri inn i en varmere ovn enn det. LES ADVARSELEN om termisk sjokk under Utstyr før du forvarmer glasset.' },
  { id:'ciabatta', lokket:false, navn:'Midt i ovnen på stål, kort og varmt', anbefaltTil:'ciabatta og andre flate, våte emner', vekt:'200–300 g biter', hydrering:'80–85 %',
  ...
  { id:'baguette', lokket:false, navn:'Nedre-midt med kraftig damp', anbefaltTil:'baguetter og avlange småbrød', vekt:'250–350 g', hydrering:'68–73 %',
  ...
  { id:'focaccia', lokket:false, navn:'I oljet form, uten damp', anbefaltTil:'focaccia og annet som hever i form', vekt:'~700 g / 25 cm form', hydrering:'75–85 %',
...
/* Forvarmingstid i minutter per stekeprofil. Flyttet hit fra app.js (V1) så
   ...
   Tung termisk masse (stål, klokke) trenger lengst; en form kortest.           */
const FORVARM_MIN = {
...
const FORMER = [
  ...
  { id:'form', navn:'Brødform', kort:'når deigen er for slapp', ikon:'▭',
    /* Brødform pekte på `apen` — åpen steking på stein ved 270 °C. Det er ikke
       hva en brødform er: den står på et brett eller en rist, uten forvarmet
       masse under seg og med vegger som bærer deigen. `brett` er nettopp det
       oppsettet, og profilen bak den steker 240 → 210 i stedet for 270. */
    kFaktor:0, maal:'', utstyr:'brett',
...
const FF_TYPER = [
  ...
  /* `kultur:true` betyr at forfermenten podes med MODEN SURDEIGSSTARTER, ikke
     med kommersiell gjær. Uten flagget doserte motoren tørrgjær i levainen —
     og et levain med tørrgjær i er ikke et levain, det er en poolish.
     `podePct` er starter i prosent av levainens eget mel. */
...
const UTSTYR = [
  ...
  /* «(referanse)» er tatt ut av navnet. Gryta er ikke fasiten — den er
     SAMMENLIGNINGSGRUNNLAGET tallene i denne lista er regnet mot, fordi
     kontakttemperaturene stammer fra støpejernets effusivitet. Det er en
     målestokk, ikke en dom over hva du bør bake i. */
  { id:'stopejern', navn:'Støpejernsgryte', effusivitet:13123, kontakt:232, forvarm:'45–60 min',
    damp:'utmerket — lukket kammer', best:'Frittstående brød, uten at du må ordne damp separat',
    om:'Løser damp og bunnvarme i én gjenstand, og det er derfor den er enkel: ingenting å rigge, ingen vannform å passe på. Tallene for de andre oppsettene i denne lista er regnet MOT støpejernet (effusivitet 13 123, 232 °C kontakttemperatur mot kald deig) — ikke fordi gryta er det beste valget for deg, men fordi den er den mest brukte målestokken i faglitteraturen og derfor gir sammenlignbare tall.' },
...
const TIPS = [
  ...
  },
  {
    ...
    punkter:[
      ...
      ['Løsningen du allerede eier','Sett glassgryta oppå det forvarmede 15 mm-stålet. Stålet leverer bunnvarmen gjennom glasset, glasset leverer dampen. Enda bedre, hvis fasongen tillater det: sett brødet DIREKTE på stålet og snu glassgryta over som en klokke. Da får du full metallkontakt og lukket dampkammer samtidig — nøyaktig oppsettet som vant King Arthurs egen metodesammenligning.'],
      ...
      ['Rangering for maks ovnsløft med ditt utstyr','1) Brød på stålet med glasset som klokke over. 2) Glassgryte stående på det forvarmede stålet. 3) Åpen steking på stålet med kokende vann i forvarmet støpejernspanne. 4) Glassgryte alene på rist — det svakeste alternativet, og sannsynligvis der du er i dag.']
    ]
  },
  {
